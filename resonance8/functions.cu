...
//Need these functions for writing to files, easier to insert than fix the GPU dicrepency
int poweri_host(int xx, int l)
        {
         int i;
         double re ;
...
void  convei_host(int zahl, char p[SSL])
        {
          int dig, j, ii = 0, sta, ani, z ;
          double dum;

          if (zahl < 0) {p[0] = 45; ++ii;} //fabs not allowed on device 
          z = fabs(zahl);

          dum = log10( (double) z);
          ani = (int) dum;
          sta = poweri_host(10,ani);
...
void writespec(int particlemax, char outdir[FILEDIM])
{
    ...
    for(i=1;i<particlemax;i++)// Cycle through the particles
    {
        if(particle[i].stable == 1) //Only print out the stable particles
        {
            ...
            convei_host(abs(particle[i].monval),p);
            strcat(filename,p);
            strcat(filename2,p);
            strcat(filename,".dat");
