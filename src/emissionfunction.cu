...
// Calculate dN_xxx array.
{
  ...
  double y = particle_y;
  particle_info* particle;			//Make particle_info pointer
  particle = &particles[particle_idx];		//Point to the right index in the particle array
      ...
      for (int j=0; j<phi_tab_length; j++)
      {
          ...
          for (long l=0; l<FO_length; l++)
          {
              ...
              for (int k=0; k<eta_tab_length; k++)
              {
                  ...
                  double f0    = 1./(exp(expon)+sign);   		//+1 for fermion, -1 for boson
